...
class RowSelfAttention(Attention):
    ...
    def __init__(
        ...
        """
        super().__init__(q_dim=q_dim, kv_dim=q_dim, c=c, n_head=n_head, out_dim=out_dim, use_bias=use_bias, gating=gating)
        self.max_tokens_per_msa = max_tokens_per_msa
        self.dropout = nn.Dropout(dropout)

    def forward(
        self,
        x: Float[torch.Tensor, "... Q q_dim"],
        ...
        Returns:
            Output tensor (..., Q, out_dim) and attention weights (optional for visualization).
        """
